...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (heroProfile) {
    gsap.set(heroProfile, { opacity: 1 });
    const tl = gsap.timeline({ delay: 0.2 });

    // 头像弹性入场
    const heroAvatar = heroProfile.querySelector('.hero-avatar');
    if (heroAvatar) {
      tl.fromTo(heroAvatar,
        { scale: 0.5, opacity: 0 },
        { scale: 1, opacity: 1, duration: 0.9, ease: 'elastic.out(1, 0.75)' }
      );
    }

    // 姓名、tagline、学校、联系方式依次渐现
    const heroIntro = heroProfile.querySelectorAll('.hero-name, .hero-tagline, .hero-school, .hero-contact-row');
    ...
    // 各 CV section 标题阶梯滑入
    const sectionTitles = heroProfile.querySelectorAll('.cv-section-title');
    if (sectionTitles.length > 0) {
      tl.fromTo(sectionTitles,
        { opacity: 0, x: -20 },
    ...
    // CV 卡片逐个滑入
    const cvCards = heroProfile.querySelectorAll('.cv-card, .cv-honor-item');
    if (cvCards.length > 0) {
      tl.fromTo(cvCards,
        { opacity: 0, y: 20 },
    ...
    // 滚动提示淡入
    const scrollHint = heroProfile.querySelector('.hero-scroll-hint');
    if (scrollHint) {
    ...
    // CV 卡片 Hover 交互
    if (config.hover_effect) {
      const cards = heroProfile.querySelectorAll('.cv-card');
      cards.forEach(card => {
        card.addEventListener('mouseenter', () => {
          gsap.to(card, {
            y: -3,
            boxShadow: '0 6px 20px var(--shadow-color1)',
            duration: 0.25,
            ease: 'power2.out'
          });
        });
        card.addEventListener('mouseleave', () => {
          gsap.to(card, {
            y: 0,
            boxShadow: '0 1px 6px var(--shadow-color2)',
            duration: 0.25,
            ease: 'power2.out'
          });
        });
      ...
      const contactChips = heroProfile.querySelectorAll('.hero-contact-chip');
      contactChips.forEach(chip => {
        chip.addEventListener('mouseenter', () => {
          gsap.to(chip, { scale: 1.06, y: -2, duration: 0.2, ease: 'power2.out' });
        });
        chip.addEventListener('mouseleave', () => {
          gsap.to(chip, { scale: 1, y: 0, duration: 0.2, ease: 'power2.out' });
        });
      });
    }
  }
